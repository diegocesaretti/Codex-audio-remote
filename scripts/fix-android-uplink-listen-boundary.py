...
# Always run final cleanup/quality wiring after every historical Android transform.
quality_script = Path(__file__).with_name("finalize-android-downlink-quality.py")
namespace = {"__file__": str(quality_script), "__name__": "__main__"}
exec(compile(quality_script.read_text(encoding="utf-8"), str(quality_script), "exec"), namespace)
