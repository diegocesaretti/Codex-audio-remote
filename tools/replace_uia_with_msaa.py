...
# The .NET 8 runner does not resolve UIAutomationClient reference assemblies by default.
# Use the older MSAA/IAccessible API instead; it is available through Windows Forms and
# is also populated by Chromium/Electron accessibility trees.

p = Path('windows/CodexAudioRemote.Server/CodexAudioRemote.Server.csproj')
s = p.read_text(encoding='utf-8')
s = s.replace('    <Reference Include="UIAutomationClient" />\n', '')
s = s.replace('    <Reference Include="UIAutomationTypes" />\n', '')
p.write_text(s, encoding='utf-8')

p = Path('windows/CodexAudioRemote.Server/CodexUiStateDetector.cs')
p.write_text(r'''using Accessibility;
...
print('MSAA Codex state detector installed')
